...
export default function GameDisplay() {
  ...
  return (
    <div ref={containerRef} className="flex flex-col h-screen bg-[#00ff00] text-white" style={{ fontFamily: 'Roboto, Arial, sans-serif' }}>
      {/* Audio elements */}
            ...
            <>
              ...
              <div className="md:w-[204px]">
                <div className="bg-white h-full p-4 flex flex-col">
                  {match.timeRemaining > 128 && <img src="/img/robot.svg" className="h-[40px]" />}
                  {match.timeRemaining <= 128 && match.timeRemaining > 120 && <img src="/img/hand.svg" className="h-[40px]" />}
                  {match.timeRemaining <= 120 && <img src="/img/gamepad.svg" className="h-[40px]" />}
                  <div className={`text-4xl md:text-7xl font-bold text-center my-auto ${getTimerColorClass()}`}>
                    {formatTime(match.timeRemaining > 128 ? match.timeRemaining - 8 :
                      match.timeRemaining > 120 ? match.timeRemaining - 120 : match.timeRemaining)}
                  </div>
                </div>
              </div>
