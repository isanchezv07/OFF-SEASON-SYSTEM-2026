...
export default function ScoreController() {    
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      toast.success('Match actualizado exitosamente');
      fetchMatchHistory();
    } catch (error) {
  ...
  const handleLoadMatch = async (match: any) => {
    if (!window.confirm('¿Estás seguro de que quieres cargar este match como el match actual? Esto reemplazará el match en curso.')) {
      return;
    }
    
    try {
      const response = await fetch(`/api/matches/${match.id}/load`, {
        method: 'POST'
      });
      
      if (response.ok) {
        toast.success('Match cargado exitosamente');
        // The socket will update the match state automatically
        setTimeout(() => {
      ...
      toast.error('Error al cargar el match');
    }
  };
  ...
  const startAnimation = () => {
    ...
    sendMessage(message);
    // Also broadcast via server socket so other devices receive it
    socket.emit('presentationShow', { match });
    setIsAnimating(true);
  ...
  const resetAnimation = () => {
    ...
    sendMessage(message);
    // Optional: could emit a reset event if clients should force-close preview
    socket.emit('presentationShow', { reset: true });
    setIsAnimating(false);
            ...
            <div className="flex gap-2">
              {!match.inProgress ? (
                <button 
                  ...
                  disabled={match.timeRemaining <= 0}
                >
                  <Play size={20} />
                  Start
                </button>
              ...
              <button 
                ...
                >
                  {match.showScores ? <EyeOff size={20} /> : <Eye size={20} />}
                  {match.showScores ? 'Hide Scores' : 'Show Scores'}
                </button>
  
                <button 
                  onClick={toggleWinnerDisplay}
                  className={`${match.timeRemaining > 0 ? 'bg-gray-400' : 'bg-yellow-600 hover:bg-yellow-700'} text-white px-4 py-2 rounded-md flex items-center gap-2`}
                  disabled={match.timeRemaining > 0}
                >
                  <Trophy size={20} />
                  {match.showWinner ? 'Hide Winner' : 'Show Winner'}
                </button>
                ...
                <motion.button
                  ...
                  className="px-4 py-2 bg-green-600/80 text-white rounded-lg backdrop-blur-md hover:bg-green-500/80 transition-colors"
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              <div className="bg-blue-700 text-white p-4 flex justify-between items-center">
                {editingTeam === 'blueTeam' ? (
                  ...
                  <>
                    <h2 className="text-xl font-bold">{match.blueTeam.name}</h2>
                    <button 
                      onClick={() => startEditingTeam('blueTeam')} 
              ...
              <div className="p-6">
                <div className="text-5xl font-bold text-center mb-6">
                  {match.blueTeam.detailedScore ? calculateScore(match.blueTeam.detailedScore, match.autoDuplicate) : 0}
                </div>
                ...
                <div className="bg-blue-50 rounded-lg p-4 mb-4">
                  {/* Fouls Section */}
                  <div className="mb-6">
                    <h3 className="text-lg font-bold mb-3 text-blue-800">Fouls</h3>
                    
                    {/* Minor Fouls */}
                    <div className="mb-4">
                      <h4 className="text-md font-semibold mb-2 text-blue-700">Minor Fouls (5 pts each)</h4>
                      <div className="flex items-center justify-between bg-white rounded-lg p-3">
                        <button
                          onClick={() => updateDetailedScore('redTeam', 'minorFoul', -1)}
                          className="w-16 h-16 bg-blue-100 hover:bg-blue-200 rounded-lg text-2xl font-bold"
                        >-</button>
                        <span className="text-2xl font-bold w-16 text-center">
                          {match.redTeam.detailedScore.minorFoul}
                        </span>
                        <button
                          onClick={() => updateDetailedScore('redTeam', 'minorFoul', 1)}
                          className="w-16 h-16 bg-blue-100 hover:bg-blue-200 rounded-lg text-2xl font-bold"
                    ...
                    {/* Major Fouls */}
                    <div className="mb-4">
                      <h4 className="text-md font-semibold mb-2 text-blue-700">Major Fouls (15 pts each)</h4>
                      <div className="flex items-center justify-between bg-white rounded-lg p-3">
                        <button
                          onClick={() => updateDetailedScore('redTeam', 'majorFoul', -1)}
                          className="w-16 h-16 bg-blue-100 hover:bg-blue-200 rounded-lg text-2xl font-bold"
                        >-</button>
                        <span className="text-2xl font-bold w-16 text-center">
                          {match.redTeam.detailedScore.majorFoul}
                        </span>
                        <button
                          onClick={() => updateDetailedScore('redTeam', 'majorFoul', 1)}
                          className="w-16 h-16 bg-blue-100 hover:bg-blue-200 rounded-lg text-2xl font-bold"
                  ...
                  {(match.timeRemaining > 120 || match.timeRemaining === 0) && (
                    <div className="mb-6">
                      <h3 className="text-lg font-bold mb-3 text-blue-800">Autonomous Period</h3>
                    
                      {/* Auto Park */}
                      <div className="mb-4">
                        <h4 className="text-md font-semibold mb-2 text-blue-700">Auto Park</h4>
                        <div className="grid grid-cols-3 gap-2">
                          {['A', 'B', 'C'].map((robot, index) => (
                            <button 
                              key={`autoPark${robot}`}
                              onClick={() => updateDetailedScore('blueTeam', `autoPark${robot}` as keyof FullScore, 0)}
                              className={`p-3 rounded-lg text-lg font-bold flex items-center justify-center gap-2 ${
                                match.blueTeam.detailedScore[`autoPark${robot}` as keyof FullScore] 
                                ? 'bg-green-500 text-white' 
                                : 'bg-gray-200 text-gray-700'
                              }`}
                            >
                              <img 
                                src={`/img/avatar/${match.blueTeam.teams[index]}.svg`}
                                onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
                                  const target = e.currentTarget;
                                  if (target.src.endsWith('.svg')) {
                                    target.src = `/img/avatar/${match.blueTeam.teams[index]}.png`;
                                  } else {
                                    target.onerror = null;
                      ...
                      {/* Auto Debris */}
                      <div className="mb-4">
                        <h4 className="text-md font-semibold mb-2 text-blue-700">Auto Debris</h4>
                        <div className="flex items-center justify-between bg-white rounded-lg p-3">
                          <button
                            onClick={() => updateDetailedScore('blueTeam', 'autoDebris', -1)}
                            className="w-16 h-16 bg-blue-100 hover:bg-blue-200 rounded-lg text-2xl font-bold"
                          >-</button>
                          <span className="text-2xl font-bold w-16 text-center">
                            {match.blueTeam.detailedScore.autoDebris}
                          </span>
                          <button
                            onClick={() => updateDetailedScore('blueTeam', 'autoDebris', 1)}
                            className="w-16 h-16 bg-blue-100 hover:bg-blue-200 rounded-lg text-2xl font-bold"
                      ...
                      {/* Auto Specimen */}
                      <div className="mb-4">
                        <h4 className="text-md font-semibold mb-2 text-blue-700">Auto Specimen</h4>
                          ...
                          {[
                            ...
                            <div key={key} className="flex items-center justify-between bg-white rounded-lg p-3">
                              ...
                              <div className="flex items-center gap-3">
                                <button
                                  onClick={() => updateDetailedScore('blueTeam', key as keyof FullScore, -1)}
                                  className="w-14 h-14 bg-blue-100 hover:bg-blue-200 rounded-lg text-xl font-bold"
                                >-</button>
                                <span className="text-2xl font-bold w-12 text-center">
                                  {match.blueTeam.detailedScore[key as keyof FullScore]}
                                </span>
                                <button
                                  onClick={() => updateDetailedScore('blueTeam', key as keyof FullScore, 1)}
                                  className="w-14 h-14 bg-blue-100 hover:bg-blue-200 rounded-lg text-xl font-bold"
                      ...
                      {/* Auto RP Toggle */}
                      <div className="mt-4">
                        <h4 className="text-md font-semibold mb-2 text-blue-700">Autonomous RP</h4>
                        <button
                          onClick={() => toggleAutoRP('blueTeam')}
                          className={`w-full p-3 rounded-lg text-lg font-bold flex items-center justify-center gap-2 ${
                            match.blueTeam.detailedScore.autoRP 
                            ? 'bg-green-500 text-white' 
                            : 'bg-gray-200 text-gray-700'
                          }`}
                        >
                          {match.blueTeam.detailedScore.autoRP ? 'Earned' : 'Not Earned'}
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {(match.timeRemaining <= 120 || match.timeRemaining === 0) && (
                    <>
                      <div className="mb-6">
                        <h3 className="text-lg font-bold mb-3 text-blue-800">Teleoperated Period</h3>
                        
                        {/* Tele Debris */}
                        <div className="mb-4">
                          <h4 className="text-md font-semibold mb-2 text-blue-700">Debris</h4>
                          <div className="flex items-center justify-between bg-white rounded-lg p-3">
                            <button
                              onClick={() => updateDetailedScore('blueTeam', 'teleDebris', -1)}
                              className="w-16 h-16 bg-blue-100 hover:bg-blue-200 rounded-lg text-2xl font-bold"
                            >-</button>
                            <span className="text-2xl font-bold w-16 text-center">
                              {match.blueTeam.detailedScore.teleDebris}
                            </span>
                            <button
                              onClick={() => updateDetailedScore('blueTeam', 'teleDebris', 1)}
                              className="w-16 h-16 bg-blue-100 hover:bg-blue-200 rounded-lg text-2xl font-bold"
                        ...
                        {/* Tele Specimen */}
                        <div>
                          <h4 className="text-md font-semibold mb-2 text-blue-700">Specimen</h4>
                          <div className="grid grid-cols-1 gap-3">
                            {[
                      ...
                      {/* Endgame */}
                      <div>
                        <h3 className="text-lg font-bold mb-3 text-blue-800">Endgame</h3>
                        
                        {/* Endgame Park */}
                        <div className="mb-4">
                          <h4 className="text-md font-semibold mb-2 text-blue-700">Robot Park/Ascent Level</h4>
                          <div className="grid grid-cols-1 gap-3">
                            {['A', 'B', 'C'].map((robot, index) => (
                              <div key={`teleopPark${robot}`} className="flex items-center justify-between bg-white rounded-lg p-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  />
                                  <span className="text-lg font-medium">{match.blueTeam.teams[index]}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                  <button
                                    onClick={() => updateDetailedScore('blueTeam', `teleopPark${robot}` as keyof FullScore, -1)}
                                    className="w-14 h-14 bg-blue-100 hover:bg-blue-200 rounded-lg text-xl font-bold"
                                  >-</button>
                                  <span className="text-2xl font-bold w-12 text-center">
                                    {match.blueTeam.detailedScore[`teleopPark${robot}` as keyof FullScore]}
                                  </span>
                                  <button
                                    onClick={() => updateDetailedScore('blueTeam', `teleopPark${robot}` as keyof FullScore, 1)}
                                    className="w-14 h-14 bg-blue-100 hover:bg-blue-200 rounded-lg text-xl font-bold"
                        ...
                        {/* Banner Points */}
                        <div>
                          <h4 className="text-md font-semibold mb-2 text-blue-700">Banner Points</h4>
                          <div className="grid grid-cols-1 gap-3">
                              ...
                              <div key={`endgameBanner${banner}`} className="flex items-center justify-between bg-white rounded-lg p-3">
                                <div className="flex items-center gap-2">
                                  <img 
                                    ...
                                    className="w-6 h-6 object-cover rounded"
                                  />
                                  <span className="text-lg font-medium">{match.blueTeam.teams[index]} Banner</span>
                                </div>
                                <div className="flex items-center gap-3">
                                  <button
                                    onClick={() => updateDetailedScore('blueTeam', `endgameBanner${banner}` as keyof FullScore, -1)}
                                    className="w-14 h-14 bg-blue-100 hover:bg-blue-200 rounded-lg text-xl font-bold"
                                  >-</button>
                                  <span className="text-2xl font-bold w-12 text-center">
                                    {match.blueTeam.detailedScore[`endgameBanner${banner}` as keyof FullScore]}
                                  </span>
                                  <button
                                    onClick={() => updateDetailedScore('blueTeam', `endgameBanner${banner}` as keyof FullScore, 1)}
                                    className="w-14 h-14 bg-blue-100 hover:bg-blue-200 rounded-lg text-xl font-bold"
